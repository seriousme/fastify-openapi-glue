...
class parserV3 {
  ...
  processPaths(paths) {
    ...
    for (let path in paths) {
      ...
      this.copyProps(data, genericSchema, copyItems);
      if (typeof data.parameters === "object") {
        this.parseParameters(genericSchema, data.parameters);
      }
      for (let pathItem in data) {
        if (HttpOperations.has(pathItem)) {
          this.processOperation(path, pathItem, data[pathItem], genericSchema);
        }
      }
    }
  }
