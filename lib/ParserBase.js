	...
	removeRecursion(schemas) {
		for (const item in schemas) {
			if (item === "response") {
				...
				const schema = structuredClone(schemas[item]);
				this.processResponseSchema(schema);
				schemas[item] = schema;
			} else {
				// some schemas are in the form of "body->content->mimeType->schema"
				if (schemas[item].content) {
					for (const contentType in schemas[item].content) {
						const schema = structuredClone(
							schemas[item].content[contentType].schema,
						);
						this.processSchema(schema);
						schemas[item].content[contentType].schema = schema;
					}
				}
