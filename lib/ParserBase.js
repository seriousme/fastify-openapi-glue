...
export class ParserBase {
	...
	processSchema(schema) {
		...
		const obj = structuredClone(schema);
		let refAdded = false;
	...
	removeRecursion(schemas) {
		for (const item in schemas) {
			if (item === "response") {
				this.processResponseSchema(schema);
			} else {
				// some schemas are in the form of "body->content->mimeType->schema"
				if (schemas[item].content) {
					const content = schemas[item].content;
					for (const contentType in content) {
						const contentTypeObj = content[contentType];
						contentTypeObj.schema = this.processSchema(contentTypeObj.schema);
					}
				}
				// all others are in the form of "query->schema" etc
				else {
					schemas[item] = this.processSchema(schemas[item]);
				}
			}
		}
	}
