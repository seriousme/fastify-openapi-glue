...
const pluginPackage = await importJSON(join(dir, "../package.json"));
const newPkg = await importJSON(join(dir, "./templates/package.json"));
...
function pathLocal(from, file) {
  ...
  return join(relative(from, join(dir, "..")), file)
    .replace(/\\/g, "/");
}
...
class generator {
  ...
  generatePlugin(specFile, service, security, dirnameModule, pluginPackageName) {
    pluginPackageName = pluginPackageName || pluginPackage.name;

    return makePlugin({ specFile, service, security, dirnameModule, pluginPackageName });
  }
  ...
  generateProject(dir, project) {
    ...
    const dirnameModule = "lib/dirname-esm.js"
    const files = {
      spec: contents(
        projectDir,
        specFile,
      ...
      plugin: contents(
        ...
        this.generatePlugin(
          ...
          serviceFile,
          securityFile,
          this.localPlugin ? pathLocal(projectDir, dirnameModule) : join(pluginPackage.name,dirnameModule),
